...
export default function Categories() {
  ...
  const createMutation = useCreateCategory({ mutation: { onSuccess: () => { refetch(); setShowCreate(false); setForm({ name: "", description: "" }); toast({ title: "Categoría creada" }); }, onError: () => toast({ title: "Error", variant: "destructive" }) } });
  const updateMutation = useUpdateCategory({ mutation: { onSuccess: () => { refetch(); setShowEdit(null); toast({ title: "Categoría actualizada" }); }, onError: () => toast({ title: "Error", variant: "destructive" }) } });
  const deleteMutation = useDeleteCategory({ mutation: { onSuccess: () => { refetch(); toast({ title: "Categoría eliminada" }); }, onError: () => toast({ title: "Error", variant: "destructive" }) } });
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-serif text-2xl font-bold">Categorías</h1>
            <p className="text-sm text-muted-foreground">{items.length} categorías</p>
          </div>
          <Button onClick={() => { setForm({ name: "", description: "" }); setShowCreate(true); }} className="gap-2"><Plus size={16} />Nueva Categoría</Button>
        </div>
      ...
      <Dialog open={!!showEdit} onOpenChange={() => setShowEdit(null)}>
        <DialogContent><DialogHeader><DialogTitle>Editar Categoría</DialogTitle></DialogHeader>
